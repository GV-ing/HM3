# 🤖 HM3 Project: FLIGHT TEST WITH DRONE AND PX4-AUTOPILOT
...
└── aerial_robotics/                                    # 
    ...
    ├── ros2_ws\src/                                    #
    |   ├──force_land\src/                              #
    |   |   └──force_land_node.cpp                      # modified file to satisfy the requirements of point 2
    |   ├──offboard_rl\src/                             #
    |   |   └──trajectory_planner.cpp                   # new file added to satisfy the requirements of point 3
    │   └──read_rpy/
    └── DDS_run.sh/      
...
## ⚙️ 2a Modified Forced-Landing Procedure. 
...
## 🔧 3a Trajectory Planner. 

For the third task, we created a new file named trajectory_planner.cpp, which implements eight waypoints that define the drone’s trajectory. The drone follows the entire sequence of waypoints without stopping at each individual point.
### 📎 How to run
```bash
make px4_sitl gz_a_100
```
